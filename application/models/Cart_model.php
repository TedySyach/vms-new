...
class Cart_model extends CI_Model
{
  ...
  function get_all()
  {
    ...
    $this->db->where('transaksi.status !=', 0); // Menggunakan operator tidak sama dengan
      $this->db->order_by('transaksi.id_trans', 'asc'); // Order by id_trans in descending order
  ...
  {
      $this->db->select('id_trans, id_invoice, users.name, transaksi.created_date, transaksi.grand_total, transaksi.status');
  
      // Menambahkan CASE WHEN untuk mengganti nilai status dengan keterangan yang diinginkan
      $this->db->select("
          CASE 
              WHEN transaksi.status = 1 THEN 'Belum Lunas'
              WHEN transaksi.status = 2 THEN 'Lunas'
              WHEN transaksi.status = 3 THEN 'Expired'
              ELSE 'Undefined'
      ...
      $this->db->order_by('transaksi.id_trans', 'desc'); // Order by id_trans in descending order
      return $this->db->get($this->table)->result();
  }
  ...
  function get_allApiBySyc($id = null)
  {
      ...
      $this->db->join('users', 'transaksi.user_id = users.id');
      $this->db->join('synchronization_plat', 'transaksi.id_sycPlat = synchronization_plat.id_syc');
      $this->db->where('transaksi.status !=', 0); // Menggunakan operator tidak sama dengan
      if ($id != null) {
        $this->db->where('synchronization_plat.id_merchat_upos',$id);
      }
       $this->db->order_by('transaksi.id_trans', 'desc'); // Order by id_trans in descending order
      return $this->db->get($this->table)->result();
  }
   ...
   // BACKEND //
  function get_allApiBySycCount($id = null)
  {
      $this->db->join('users', 'transaksi.user_id = users.id');
      $this->db->join('synchronization_plat', 'transaksi.id_sycPlat = synchronization_plat.id_syc');
      $this->db->where('transaksi.tipe_trx', 1);
  ...
  function get_allApiOrg($id = null)
  {
      ...
      $this->db->where('transaksi.status !=', 0); // Menggunakan operator tidak sama dengan
      if ($id != null) {
        $this->db->where('synchronization_plat.id_organize_daftarin',$id);
      }
      $this->db->order_by('transaksi.id_trans', 'desc'); // Order by id_trans in descending order
      return $this->db->get($this->table)->result();
  }
  ...
  function get_cart_per_customer_finished_back($id)
  {
   ...
   function get_cart_per_Trans_finished_backApi($id=null)
  {
    $this->db->select('
    lapangan.id_lapangan, lapangan.nama_lapangan,
    transaksi.id_trans, transaksi.id_invoice, transaksi.user_id, transaksi.subtotal, transaksi.diskon, transaksi.grand_total, transaksi.deadline, transaksi.status, transaksi.catatan, transaksi.created_date,
  ...
  // cek transaksi  callback
  function cek_transaksiCallBack($inv)
  {
  ...
  function get_cart_per_customer()
  {
    ...
    $this->db->join('transaksi', 'transaksi_detail.trans_id = transaksi.id_trans');
    $this->db->join('users', 'transaksi.user_id = users.id');
    $this->db->where('transaksi.user_id', $idCtm);
    $this->db->where('status','0');
    return $this->db->get($this->table2);
  }
  ...
  public function transaksiUpdate($data,$id) {
